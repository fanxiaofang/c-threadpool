...
struct threadpool_t {
  ...
  pthread_cond_t notify;          // 条件变量，通知线程任务变化
  ...
  int count;                     // 当前任务数量
  int shutdown;                  // 线程池关闭标志（0：运行，1或2：关闭）
  int started;                   // 当前存活线程数
};
...
- 检查线程池的关闭条件(立即关闭或者无优雅关闭且无任务)，减少线程池的存货线程数`started--`,释放互斥锁`lock`,退出线程`pthread_exit`
...
如何进一步完善:<br>
1. 动态调整线程数量、动态调整任务队列大小（待定）
2. 允许任务队列满时进行阻塞等待，避免满队列时任务丢失
...
4. 增加发送信号暂停线程，支持暂停/恢复（通过SIGUSER1自定义信号）
